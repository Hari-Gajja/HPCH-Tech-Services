...
function App() {
  ...
  // OTP Email Validation State
  const [otpState, setOtpState] = useState({
    sent: false,         // OTP has been sent
    verified: false,     // Email verified successfully
    sending: false,      // Currently sending OTP
    verifying: false,    // Currently verifying OTP
    error: '',           // Error message
    successMsg: '',      // Success message
    otpValue: '',        // User-entered OTP
    email: '',           // Email the OTP was sent to
    cooldown: 0,         // Resend cooldown seconds
    previewUrl: '',      // Ethereal preview URL
    hash: '',            // HMAC hash for stateless verification
    expiresAt: 0,        // OTP expiry timestamp
  })
  const cooldownTimerRef = useRef(null)

  // Phone OTP Validation State (Twilio)
  ...
  const handleSendPhoneOtp = async () => {
    const phoneInput = formRef.current?.from_phone
    const phone = phoneInput?.value?.trim()

    if (!phone || phone.replace(/[\s\-\(\)\+]/g, '').length < 10) {
      setPhoneOtpState(prev => ({ ...prev, error: 'Please enter a valid phone number first.', successMsg: '' }))
      setTimeout(() => setPhoneOtpState(prev => ({ ...prev, error: '' })), 4000)
      return
    }

    setPhoneOtpState(prev => ({ ...prev, sending: true, error: '', successMsg: '' }))
    ...
    try {
      ...
      if (res.ok) {
        setPhoneOtpState(prev => ({
          ...
          sending: false,
          phone: data.phone,
          successMsg: 'OTP sent to your phone!',
          cooldown: 60,
          hash: data.hash,
        ...
        phoneCooldownRef.current = setInterval(() => {
          setPhoneOtpState(prev => {
            if (prev.cooldown <= 1) {
              clearInterval(phoneCooldownRef.current)
        ...
        setPhoneOtpState(prev => ({ ...prev, sending: false, error: data.error || 'Failed to send OTP.' }))
      }
    } catch {
      setPhoneOtpState(prev => ({ ...prev, sending: false, error: 'Network error. Please try again.' }))
    }
  }

  // Verify phone OTP via Twilio
  const handleVerifyPhoneOtp = async () => {
    if (!phoneOtpState.otpValue || phoneOtpState.otpValue.length !== 6) {
      setPhoneOtpState(prev => ({ ...prev, error: 'Please enter the 6-digit OTP.', successMsg: '' }))
      setTimeout(() => setPhoneOtpState(prev => ({ ...prev, error: '' })), 4000)
      return
    }

    setPhoneOtpState(prev => ({ ...prev, verifying: true, error: '', successMsg: '' }))

    try {
      const res = await fetch('/api/verify-phone-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone: phoneOtpState.phone, code: phoneOtpState.otpValue, hash: phoneOtpState.hash, expiresAt: phoneOtpState.expiresAt }),
      })
      const data = await res.json()

      if (res.ok) {
        setPhoneOtpState(prev => ({ ...prev, verified: true, verifying: false, successMsg: 'Phone verified successfully!', error: '' }))
        if (phoneCooldownRef.current) clearInterval(phoneCooldownRef.current)
      } else {
        setPhoneOtpState(prev => ({ ...prev, verifying: false, error: data.error || 'Invalid OTP.' }))
      }
    } catch {
      setPhoneOtpState(prev => ({ ...prev, verifying: false, error: 'Network error. Please try again.' }))
    }
  }

  // Reset phone OTP state when phone number changes
  const handlePhoneChange = () => {
    if (phoneOtpState.sent || phoneOtpState.verified) {
      setPhoneOtpState({
        sent: false, verified: false, sending: false, verifying: false,
        error: '', successMsg: '', otpValue: '', phone: '', cooldown: 0, hash: '', expiresAt: 0,
      })
      if (phoneCooldownRef.current) clearInterval(phoneCooldownRef.current)
    }
  }
  ...
  const handleSendOtp = async () => {
    const emailInput = formRef.current?.from_email
    const email = emailInput?.value?.toLowerCase().trim()

    if (!email || !isValidEmail(email)) {
      setOtpState(prev => ({ ...prev, error: 'Please enter a valid email address first.', successMsg: '' }))
      setTimeout(() => setOtpState(prev => ({ ...prev, error: '' })), 4000)
      return
    }

    setOtpState(prev => ({ ...prev, sending: true, error: '', successMsg: '', previewUrl: '' }))
    ...
    try {
      ...
      if (res.ok) {
        setOtpState(prev => ({
          ...
          sending: false,
          email,
          successMsg: 'OTP sent! Check your inbox (and spam folder).',
          cooldown: 60,
          hash: data.hash,
          expiresAt: data.expiresAt,
        }))
        ...
        setOtpState(prev => ({ ...prev, sending: false, error: data.error || 'Failed to send OTP.' }))
      }
    } catch {
      setOtpState(prev => ({ ...prev, sending: false, error: 'Network error. Please try again.' }))
    }
  }
  ...
  const handleVerifyOtp = async () => {
    if (!otpState.otpValue || otpState.otpValue.length !== 6) {
      setOtpState(prev => ({ ...prev, error: 'Please enter the 6-digit OTP.', successMsg: '' }))
      setTimeout(() => setOtpState(prev => ({ ...prev, error: '' })), 4000)
      return
    }
    ...
    try {
      const res = await fetch('/api/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: otpState.email, otp: otpState.otpValue, hash: otpState.hash, expiresAt: otpState.expiresAt }),
      })
      const data = await res.json()

      if (res.ok) {
        setOtpState(prev => ({ ...prev, verified: true, verifying: false, successMsg: 'Email verified successfully!', error: '' }))
        if (cooldownTimerRef.current) clearInterval(cooldownTimerRef.current)
      } else {
        setOtpState(prev => ({ ...prev, verifying: false, error: data.error || 'Invalid OTP.' }))
      }
    } catch {
      setOtpState(prev => ({ ...prev, verifying: false, error: 'Network error. Please try again.' }))
    }
  }
  ...
  const handleEmailChange = () => {
    if (otpState.sent || otpState.verified) {
      setOtpState({
        sent: false, verified: false, sending: false, verifying: false,
        error: '', successMsg: '', otpValue: '', email: '', cooldown: 0, previewUrl: '', hash: '', expiresAt: 0,
      })
      if (cooldownTimerRef.current) clearInterval(cooldownTimerRef.current)
    }
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (!otpState.verified) {
      setFormStatus({ loading: false, success: false, error: false, blocked: false, invalid: false })
      setOtpState(prev => ({ ...prev, error: 'Please verify your email with OTP before sending.' }))
      setTimeout(() => setOtpState(prev => ({ ...prev, error: '' })), 5000)
      return
    }

    // Check if phone is verified via Twilio OTP
    if (!phoneOtpState.verified) {
      setFormStatus({ loading: false, success: false, error: false, blocked: false, invalid: false })
      setPhoneOtpState(prev => ({ ...prev, error: 'Please verify your phone number with OTP before sending.' }))
      setTimeout(() => setPhoneOtpState(prev => ({ ...prev, error: '' })), 5000)
      return
    }
    ...
    .then(() => {
      ...
      setOtpState({ sent: false, verified: false, sending: false, verifying: false, error: '', successMsg: '', otpValue: '', email: '', cooldown: 0, previewUrl: '' })
      if (cooldownTimerRef.current) clearInterval(cooldownTimerRef.current)
      // Reset phone OTP state
      setPhoneOtpState({ sent: false, verified: false, sending: false, verifying: false, error: '', successMsg: '', otpValue: '', phone: '', cooldown: 0 })
      if (phoneCooldownRef.current) clearInterval(phoneCooldownRef.current)
      setTimeout(() => setFormStatus({ loading: false, success: false, error: false, blocked: false, invalid: false }), 5000)
    })
    .catch(() => {
                ...
                <div className="field">
                  ...
                  <div className="email-otp-row">
                    ...
                    {!otpState.verified && (
                      <button
                        type="button"
                        className="btn otp-send-btn"
                        onClick={handleSendOtp}
                        disabled={otpState.sending || otpState.cooldown > 0}
                      >
                        <i className={otpState.sending ? 'fas fa-spinner fa-spin' : 'fas fa-paper-plane'}></i>
                        {otpState.sending ? 'Sending...' : otpState.cooldown > 0 ? `Resend (${otpState.cooldown}s)` : otpState.sent ? 'Resend OTP' : 'Validate'}
                      </button>
                    )}
                    {otpState.verified && (
                      <span className="otp-verified-badge"><i className="fas fa-check-circle"></i> Verified</span>
                    )}
                  </div>
                  {/* OTP Input Section */}
                    ...
                    <div className="otp-input-section">
                      <div className="otp-input-row">
                        <input
                          ...
                          onChange={(e) => setOtpState(prev => ({ ...prev, otpValue: e.target.value.replace(/\D/g, '').slice(0, 6) }))}
                        />
                        <button
                          type="button"
                          className="btn otp-verify-btn"
                          onClick={handleVerifyOtp}
                          disabled={otpState.verifying || otpState.otpValue.length !== 6}
                        >
                          <i className={otpState.verifying ? 'fas fa-spinner fa-spin' : 'fas fa-shield-alt'}></i>
                  ...
                  {otpState.error && (
                    <div className="otp-message otp-error">
                      <i className="fas fa-exclamation-circle"></i> {otpState.error}
                    </div>
                  )}
                  {otpState.successMsg && (
                    <div className="otp-message otp-success">
                      <i className="fas fa-check-circle"></i> {otpState.successMsg}
                    </div>
                  )}
                </div>
                <div className="field">
                  ...
                  <div className="email-otp-row">
                    ...
                    {!phoneOtpState.verified && (
                      <button
                        type="button"
                        className="btn otp-send-btn phone-otp-btn"
                        onClick={handleSendPhoneOtp}
                        disabled={phoneOtpState.sending || phoneOtpState.cooldown > 0}
                      >
                        <i className={phoneOtpState.sending ? 'fas fa-spinner fa-spin' : 'fas fa-sms'}></i>
                        {phoneOtpState.sending ? 'Sending...' : phoneOtpState.cooldown > 0 ? `Resend (${phoneOtpState.cooldown}s)` : phoneOtpState.sent ? 'Resend OTP' : 'Validate'}
                      </button>
                    )}
                    {phoneOtpState.verified && (
                      <span className="otp-verified-badge"><i className="fas fa-check-circle"></i> Verified</span>
                    ...
                    <div className="otp-input-section">
                      <div className="otp-input-row">
                        <input
                          ...
                          placeholder="Enter 6-digit OTP"
                          maxLength={6}
                          value={phoneOtpState.otpValue}
                          onChange={(e) => setPhoneOtpState(prev => ({ ...prev, otpValue: e.target.value.replace(/\D/g, '').slice(0, 6) }))}
                        />
                        <button
                          type="button"
                          className="btn otp-verify-btn"
                          onClick={handleVerifyPhoneOtp}
                          disabled={phoneOtpState.verifying || phoneOtpState.otpValue.length !== 6}
                        >
                          <i className={phoneOtpState.verifying ? 'fas fa-spinner fa-spin' : 'fas fa-shield-alt'}></i>
                          {phoneOtpState.verifying ? 'Verifying...' : 'Verify OTP'}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {phoneOtpState.error && (
                    <div className="otp-message otp-error">
                      <i className="fas fa-exclamation-circle"></i> {phoneOtpState.error}
                    </div>
                  )}
                  {phoneOtpState.successMsg && (
                    <div className="otp-message otp-success">
                      <i className="fas fa-check-circle"></i> {phoneOtpState.successMsg}
                    </div>
                  )}
                </div>
                <div className="field">
